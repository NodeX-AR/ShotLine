...
export class Arena extends DurableObject {
  ...
  async webSocketClose(ws) {
    const s = this.sessions.get(ws);
    if (s && s.joined) this.broadcast({ t: "left", id: s.id, name: s.name }, ws);
  ...
  onMap(ws, msg) {
    ...
    for (let i = 0; i < n; i++) {
      const o = i * 6;
      this.boxes.push({ x0:f[o], x1:f[o+1], y0:f[o+2], y1:f[o+3], z0:f[o+4], z1:f[o+5] });
    }
    this.mapReady = true;
  ...
  onSpawn(ws, s) {
    s.hp = 100; s.alive = true; s.spawnedAt = Date.now();
    this.broadcast({
      t:"spawned", id:s.id, name:s.name, color:s.color,
      x:s.x, y:s.y, z:s.z, yaw:s.yaw,
    }, ws);
