...
function lots(cx,cz,dtn,res){for(const [sx,sz] of [[-1,-1],[1,-1],[-1,1],[1,1]]){const lcx=cx+sx*CELL/2,lcz=cz+sz*CELL/2,bw=CELL-wr(0.4,1.4),bd=CELL-wr(0.4,1.4),rr=rand();const floors=dtn?wpick([2,3,3,4,5,6,7]):res?wpick([1,2,2,3]):wpick([1,1,2]);if(rr<(dtn?0.30:0.24)&&floors>=2)enterBuilding(lcx,lcz,bw,bd,Math.min(floors,3),sx,sz);else if(rr<0.92)facade(lcx,lcz,bw,bd,floors,sx,sz);else ruin(lcx,lcz,bw,bd);}for(let k=0;k<4;k++){const ax=cx+wr(-BLOCK/2,BLOCK/2),az=cz+wr(-BLOCK/2,BLOCK/2);if(!isFree(ax,az,1.5))continue;if(rand()<0.4)crateStack(ax,az);else if(rand()<0.6)barrel(ax,az);else dumpster(ax,az);}}
          ...
          if(o.material){
            ...
            o.material.onBeforeCompile=(shader)=>{
              shader.vertexShader='varying float vBone;\n'+shader.vertexShader.replace(
                ...
                `#include <begin_vertex>
                 float mW=0.0; float mI=-1.0;
                 if(skinWeight.x>mW){mW=skinWeight.x;mI=skinIndex.x;}
                 if(skinWeight.y>mW){mW=skinWeight.y;mI=skinIndex.y;}
                 if(skinWeight.z>mW){mW=skinWeight.z;mI=skinIndex.z;}
                 if(skinWeight.w>mW){mW=skinWeight.w;mI=skinIndex.w;}
                 vBone=mI;`
              );
              shader.fragmentShader='varying float vBone;\n'+shader.fragmentShader.replace(
                'void main() {',
                `void main() {
                  bool keep=false;
                  ${conds}
...
const CH=(function(){
  ...
  const TINTS=[0xffffff,0xd8d4cc,0xc8c4bc,0xb8b4ac,0xa8a4a0,0x909090];
...
const chatHist=[];
const rctx=$('radar').getContext('2d');const RS=150,RR=45,RK=RS/2/RR;
...
function syncMeshes(dt){
  for(const b of fighters){
    if((b.isPlayer&&viewMode==='fp')||!b.mesh)continue;const m=b.mesh;b.animT+=dt;
    if(b.rlT>0)b.rlT-=dt;
    ...
    if(b.alive){m.rotation.x=0;m.rotation.z=0;
      ...
      let pt=0;if(b.remote)pt=b.pitch||0;else if(b.isPlayer)pt=pitch;else if(b.target&&b.target.alive){const tx=b.target.x-b.x,tz=b.target.z-b.z,dd=Math.hypot(tx,tz)||1;pt=Math.atan2((b.target.y+1.1)-(b.y+1.45),dd);}
      ...
      if(!b.isPlayer){const d=Math.hypot(b.x-camera.position.x,b.z-camera.position.z);b.label.visible=d<60&&!b.remote;}}
    else{
      ...
      b.deathT+=dt;
      const t=Math.min(1, b.deathT/0.85);
      const e=1-Math.pow(1-t,3);
      const s=b.deathSign||-1;
      const side=(b.name.charCodeAt(0)%2)?1:-1;
      m.rotation.x = s * e * 1.50;
      m.rotation.z = side * e * 0.22;
      m.position.y = b.y - e * 0.10;
      if(b.rig && b.rig.model && b.rig.model.bones){
        const bn=b.rig.model.bones;
        if(bn.spine)  bn.spine.rotation.x  = e * 0.35 * s;
        if(bn.spine1) bn.spine1.rotation.x = e * 0.25 * s;
        if(bn.spine2) bn.spine2.rotation.x = e * 0.20 * s;
        if(bn.head){ bn.head.rotation.x = e * 0.45 * s; bn.head.rotation.z = side * e * 0.35; }
        if(bn.rA){ bn.rA.rotation.z = -e * 0.85; bn.rA.rotation.x = e * 0.30; }
        if(bn.lA){ bn.lA.rotation.z =  e * 0.85; bn.lA.rotation.x = e * 0.30; }
...
function leaveMatch(msg){
  ...
  player.alive=false;player.hp=100;if(player.mesh)player.mesh.visible=false;
...
document.addEventListener('keydown',e=>{if(chatOpen)return;if(!$('adminPrompt').classList.contains('hidden'))return;
  ...
  keys[e.code]=true;
  if(state==='playing'&&!paused()&&!e.repeat){for(let i=1;i<=8;i++){if(e.code===BINDS['w'+i])setGun(i-1);}if(e.code===BINDS.view)toggleViewMode();}});
...
function spawn(f){
  ...
  if(f.rig && f.rig.model && f.rig.model.mixer){
    ...
    ['idle','walk','run'].forEach(n=>{
      ...
      M.acts[n].reset();
      M.acts[n].play();
      M.acts[n].setEffectiveWeight(n==='idle'?1:0);
    });
  }
...
const optViewTP=$('optViewTP');
if(optViewTP)optViewTP.addEventListener('change',()=>setViewMode(optViewTP.checked?'tp':'fp'));
...
(function(){const hud=$('mobileHud');if(!hud)return;let drag=null;
  hud.addEventListener('touchstart',e=>{if(!layoutMode)return;const t=e.target.closest('.mbtn, .mw');if(!t)return;e.preventDefault();e.stopPropagation();
    ...
    drag={el:t,id,sx:e.touches[0].clientX,sy:e.touches[0].clientY,l0:r.left,t0:r.top,t0ms:performance.now(),moved:false,isExit:t.id==='mExit'};},{capture:true,passive:false});
  ...
  document.addEventListener('touchend',()=>{if(!drag)return;
    // A plain tap on the pause/✕ button must always work, even while
    // repositioning buttons in layout-edit mode — only a real drag (moved
    // past the threshold) reassigns its position instead.
    if(drag.isExit&&!drag.moved&&performance.now()-drag.t0ms<400){manualPause=true;syncPause();}
    else{const r=drag.el.getBoundingClientRect();layoutPositions[drag.id]={left:r.left,top:r.top};layoutSave();}
    drag=null;});
})();
